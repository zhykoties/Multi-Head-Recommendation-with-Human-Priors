...
def build_dataloader(config, interaction_data, log_detailed_results=False):
    '''
    ...
    dataset_dict = {
        ...
        'ComiRec': ('SEQTrainDataset', 'SeqEvalDataset', 'seq_eval_collate'),
        'REMI': ('SEQTrainDataset', 'SeqEvalDataset', 'seq_eval_collate'),
        'DualVAE': ('SEQTrainDataset', 'SeqEvalDataset', 'seq_eval_collate'),
    ...
    train_data = train_set_class(config, interaction_data)
    valid_data = test_set_class(config, interaction_data, phase='valid')
    test_data = test_set_class(config, interaction_data, phase='test')
    ...
    train_loader = DataLoader(train_data, batch_size=config['train_batch_size'], num_workers=num_workers,
                              ...
                              collate_fn=train_collate_fn or None, worker_init_fn=init_fn)
    ...
    test_loader = DataLoader(test_data, batch_size=config['eval_batch_size'], num_workers=num_workers,
                             pin_memory=True, sampler=None if log_detailed_results else test_sampler, collate_fn=eval_collate_fn)
...
class NonConsecutiveSequentialDistributedSampler(torch.utils.data.sampler.Sampler):
    ...
    def __iter__(self):
        # yields rank, rank+num_replicas, ...
        n = len(self.dataset)
        for i in range(self.rank, n, self.num_replicas):
            yield i
