...
class AsymmetricLoss(torch.nn.Module):
    ...
    def __init__(
        ...
        """
        Args:
            gamma_pos: float, down weight for easy positive samples.
            gamma_neg: float, down weight for easy negative samples. Typically value [2, 4]
            clip: probability clip for negative samples
            eps: float, label smoothing.
            reduction: str, the reduction method for the loss. Can be 'none', 'mean' or 'sum'
        """
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x: logits, [num_candidates, num_tasks]
            y: binarized labels, [num_candidates, num_tasks]
        Returns:
            loss: [1, num_tasks]
        """
...
class SimpleOutput:
    """A small container to mimic HF model outputs with 'hidden_states'."""
    def __init__(self, hidden_states):
        # hidden_states is expected to be a list of [layer0, layer1, ... , final]
...
class DummyLLM(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.input_layer = nn.Embedding(input_size, hidden_size)
        self.embed_layer = nn.Linear(hidden_size, hidden_size)
        self.hidden_size = hidden_size

    def forward(self, input_ids=None, inputs_embeds=None, **kwargs):
        if input_ids is not None:
            assert inputs_embeds is None, "Cannot pass both input_ids and inputs_embeds"
            out = self.input_layer(input_ids)  # [batch_size, seq_len, hidden_size]
        elif inputs_embeds is not None:
            out = inputs_embeds  # user-supplied embeddings
        else:
            raise ValueError('You must provide either input_ids or inputs_embeds')

        out = self.embed_layer(out)
        return SimpleOutput(hidden_states=[out])
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask):
        ...
        attention_probs = self.softmax(attention_scores).to(value_layer.dtype)
