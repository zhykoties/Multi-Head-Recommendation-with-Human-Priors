...
class AbstractMetric(object):
    ...
    def __init__(self, config):
        self.num_prior_categories = config["eval_num_cats"]
...
class TopkMetric(AbstractMetric):
    ...
    def topk_result(self, metric, value, num_samples=None, prefix=None):
        ...
        """
        metric_dict = {}
        avg_result = value.sum(axis=0)  # will later be divided by num_users in trainer
        for k in self.topk:
            if prefix is None:
                key = f'{metric}@{k}'
            else:
                key = f'{prefix}-{metric}@{k}'
            # metric_dict[key] = round(avg_result[k - 1], self.decimal_place)
            if num_samples is not None:
                metric_dict[key] = (avg_result[k - 1], num_samples)
            else:
...
class LossMetric(AbstractMetric):
    ...
    implement a loss based metric, you can inherit this class.
