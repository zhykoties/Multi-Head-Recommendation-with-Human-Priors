...
# TopK Metrics
class Entropy(AbstractMetric):
    r"""Diversity of the recommended items.
    ...
    def calculate_metric(self, dataobject, pred_len=1):
        rec_tags = dataobject.get('rec.rec_tags')  # rec_tags: [all_users, topk, num_cats]
        tag_counts = np.cumsum(rec_tags * 1.0, axis=1)  # tag_counts: [all_users, topk, num_cats]

        metric_dict = {}
        for topk in self.topk:
            # https://stackoverflow.com/questions/33607071/fastest-way-to-compute-entropy-of-each-numpy-array-row
            p = tag_counts[:, topk-1, :] / tag_counts[:, topk-1, :].sum(axis=1, keepdims=True)
            entropy_per_user = -np.sum(
                np.where(p > 0, p * np.log2(p), 0.0), axis=1)
            metric_dict[f'Entropy@{topk}'] = entropy_per_user.sum(axis=0)
...
class Recall(TopkMetric):
    ...
    def calculate_metric(self, dataobject, pred_len=1):
        ...
        if self.num_prior_categories > 1 and self.eval_by_cat:
            tag_categories = dataobject.get('rec.tgt_tags')
            for tag_idx in range(self.num_prior_categories):
                # tag_categories: [all_users, num_categories]
                tag_mask = tag_categories[:, tag_idx]
                result = self.metric_info(pos_index[tag_mask], pos_len[tag_mask])
                metric_dict.update(self.topk_result('recall', result, num_samples=tag_mask.sum().item(),
                                                    prefix=self.int_to_category[tag_idx]))
        if self.outlier_user_metrics is not None and pred_len == self.eval_pred_len - 1:
            outlier_users = dataobject.get('rec.outlier_users')
            print(f'outlier_users: {outlier_users.shape}, sum={outlier_users.sum()}')
            result = self.metric_info(pos_index[outlier_users], pos_len[outlier_users])
            metric_dict.update(self.topk_result('recall', result, num_samples=outlier_users.sum().item(),
                                                prefix=f'outlier_{self.outlier_user_metrics}'))
        return metric_dict

    def metric_info(self, pos_index, pos_len=None):
        assert pos_len is not None
    ...
    def calculate_metric(self, dataobject, pred_len=1):
        ...
        if self.num_prior_categories > 1 and self.eval_by_cat:
            ...
            for tag_idx in range(self.num_prior_categories):
                ...
                tag_mask = tag_categories[:, tag_idx]
                result = self.metric_info(pos_index[tag_mask], pos_len[tag_mask])
                metric_dict.update(self.topk_result('ndcg', result, num_samples=tag_mask.sum().item(),
                                                    prefix=self.int_to_category[tag_idx]))
        if self.outlier_user_metrics is not None and pred_len == self.eval_pred_len - 1:
            outlier_users = dataobject.get('rec.outlier_users')

            result = self.metric_info(pos_index[outlier_users], pos_len[outlier_users])
            metric_dict.update(self.topk_result('ndcg', result, num_samples=outlier_users.sum().item(),
                                                prefix=f'outlier_{self.outlier_user_metrics}'))
        return metric_dict
